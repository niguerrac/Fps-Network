...
// This camera is similar to the one used in Jak & Dexter
	...
	{
		if (target) {
			Collider characterController = target.GetComponent<Collider>();
	...
	void SetUpRotation (Vector3 centerPos, Vector3 headPos)
	{
		...
		Ray centerRay = GetComponent<Camera>().ViewportPointToRay (new Vector3 (0.5f, 0.5f, 1));
		Ray topRay = GetComponent<Camera>().ViewportPointToRay (new Vector3 (0.5f, clampHeadPositionScreenSpace, 1));
